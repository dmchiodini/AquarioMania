...
namespace AquarioMania.Utils;

public class SystemUtils

{
    public string GenerateToken(string privateKey, int userId, string userName, string email, int profile_id)
    {
        ...
        new Claim("id", userId.ToString()),
         new Claim("profile_id", profile_id.ToString())
        });
    ...
    public DecodeTokenModel GetDecodeToken(string token, string secret)
    {
        ...
        if (IsValidToken(token, secret))
        {
            foreach (Claim claim in jwtSecurityToken.Claims)
            {
                ...
                else if (claim.Type == "profile_id")
                {
                    decodedToken.Profile_id = Convert.ToInt32(claim.Value);
                }
            }
